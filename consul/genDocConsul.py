#!/usr/bin/env python
# How to use
# > python genDocDb.consul.py -h localhost -p 8500  -d /a/b/c -n 100 -r 1
  ...
  for n in range(0,args.number):
    ...
    if args.report > 0:
      if (time.time() - t) > args.report:
        print "rtDummy "+str(n+1)+" :  w_ok:"+str(w_ok)+" w_fail:"+str(w_fail)
        t = time.time()
  print 'time: '+str(time.time()-t)+'s'
  print "rtDummy "+str(args.number)+" :  w_ok:"+str(w_ok)+" w_fail:"+str(w_fail)
  sys.exit()
